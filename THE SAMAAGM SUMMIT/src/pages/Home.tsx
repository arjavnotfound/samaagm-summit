...
export default function Home() {
  ...
  function handleArjavClick() {
    ...
    devClickTimer.current = setTimeout(() => { devClickCount.current = 0; }, DEV_CLICK_WINDOW);
    if (devClickCount.current >= DEV_CLICK_THRESHOLD) {
      devClickCount.current = 0;
      if (devClickTimer.current) clearTimeout(devClickTimer.current);
      setDevPassword(""); setDevPasswordError(""); setShowDevOverlay(true);
    }
  }
  ...
  async function handleDevPasswordSubmit(e: React.FormEvent) {
    ...
    if (hashHex === DEV_PASS_HASH) {
      setDevPasswordError(""); setShowDevOverlay(false); setDevAuthenticated(true);
    } else {
      setDevPasswordError("Incorrect password."); setDevPassword(""); devPasswordInputRef.current?.focus();
    }
  }

  function handleDevClose() {
    setDevAuthenticated(false); setShowDevOverlay(false); setDevPassword(""); setDevPasswordError("");
  }
  ...
  return (
    <div className="h-root">
      ...
      {showDevOverlay && !devAuthenticated && (
        <div className="devpw-overlay" onClick={(e) => { if (e.target === e.currentTarget) { setShowDevOverlay(false); setDevPassword(""); setDevPasswordError(""); } }}>
          <form className="devpw-modal" onSubmit={handleDevPasswordSubmit}>
            ...
            <div className="devpw-field">
              <input ref={devPasswordInputRef} type="password" className="devpw-input" placeholder="Enter access key" value={devPassword}
                onChange={(e) => { setDevPassword(e.target.value); setDevPasswordError(""); }} autoComplete="off" spellCheck={false} />
            </div>
            {devPasswordError && <p className="devpw-error">{devPasswordError}</p>}
            <button type="submit" className="devpw-submit">Authenticate →</button>
          </form>
        </div>
      )}
      ...
      <div className="h-bg-fx" aria-hidden>
        ...
        {[...Array(22)].map((_, i) => (
          <div key={i} className={`h-ptcl ${i % 5 === 0 ? "h-ptcl--gold" : ""}`}
            style={{ left: `${(i * 4.5 + 3) % 100}%`, width: `${1.5 + (i % 3) * 1.5}px`, height: `${1.5 + (i % 3) * 1.5}px`, animationDuration: `${7 + (i % 9) * 2}s`, animationDelay: `${(i * 0.7) % 8}s` }} />
        ))}
        <svg className="h-orbital" aria-hidden viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
          <ellipse cx="300" cy="300" rx="260" ry="95" fill="none" stroke="rgba(204,0,0,0.5)" strokeWidth="1" />
          <ellipse cx="300" cy="300" rx="170" ry="255" fill="none" stroke="rgba(204,0,0,0.35)" strokeWidth="1" />
        </svg>
      </div>
      ...
      <header className={`h-nav${scrolled ? " h-nav--solid" : ""}`}>
        <div className="h-nav-inner">
          ...
          <button className="h-reg-btn" onClick={goEvent}>Platform 9¾ →</button>

          <button className={`h-ham${menuOpen ? " h-ham--open" : ""}`} aria-label={menuOpen ? "Close" : "Menu"} onClick={() => setMenuOpen((o) => !o)}>
            <span /><span /><span />
          </button>
        </div>
        ...
        {menuOpen && (
          <div className="h-mobile-menu">
            {NAV_ITEMS.map(({ id, label, Icon }) => (
              <button key={id} className="h-mobile-link" onClick={() => { scrollTo(id); setMenuOpen(false); }}>
                <span className="h-mobile-icon"><Icon size={14} strokeWidth={1.5} /></span>
                {label}
              </button>
            ))}
            <a className="h-mobile-link" href="https://www.instagram.com/thesamaagmsummit.tss" target="_blank" rel="noopener noreferrer">
              <span className="h-mobile-icon"><Instagram size={14} strokeWidth={1.5} /></span>
              Instagram
            </a>
            <button className="h-reg-btn" style={{ marginTop: 16, width: "100%" }} onClick={() => { goEvent(); setMenuOpen(false); }}>
              Platform 9¾
            </button>
          </div>
        )}
      ...
      <section className="h-hero">
        <div className="h-corner h-corner--tl"><span>THE SAMAAGM SUMMIT</span></div>
        <div className="h-corner h-corner--tr"><span>YOUTH · MUN · INNOVATION</span></div>
        <div className="h-corner h-corner--bl"><span>YOUTH · MUN · INNOVATION</span></div>
        <div className="h-corner h-corner--br"><span>INDIA'S FIRST DEMOCRATIC SUMMIT</span></div>

        <div className="h-side-text h-side-text--l" aria-hidden><span>THE SAMAAGM SUMMIT — TSS —</span></div>
        <div className="h-side-text h-side-text--r" aria-hidden><span>YOUTH · MUN · INNOVATION · TSS</span></div>
          ...
          <h1 className="h-hero-title">
            ...
            <span className="h-hero-main">
              {"Samaagm".split("").map((ch, i) => (
                <span key={i} className="h-hero-ch" style={{ animationDelay: `${0.05 + i * 0.055}s` }}>{ch}</span>
              ))}
            </span>
        ...
        <div className="h-marquee-track">
          {[...MARQUEE_TEXT, ...MARQUEE_TEXT, ...MARQUEE_TEXT].map((t, i) => (
            <span key={i} className="h-marquee-item">
              <span className="h-marquee-dot">✦</span>{t}
            </span>
          ))}
        </div>
      </div>
      ...
      <section id="about" className="h-section">
        <div className="h-wrap">
          <div className="h-section-header h-reveal">
            ...
            <h2 className="h-section-title">More than<br />a conference.</h2>
          </div>

          <div className="h-about-grid">
            <div className="h-about-body">
              <p className="h-p h-reveal">
                The Samaagm Summit is a youth-driven conference designed to bring together students, thinkers, and future leaders onto one platform. Built around meaningful dialogue, collaboration, and real-world problem solving.
              </p>
              <p className="h-p h-reveal">
                At its core, TSS hosts a <strong>Model United Nations (MUN)</strong> — where participants take on roles of global representatives and engage in structured debates on pressing international and social issues.
              </p>
              <p className="h-p h-reveal">
                The Samaagm Summit is not just another conference — it is an experience that combines intellectual discussion with engagement, participation, and innovation.
              </p>
      ...
      <section id="vision" className="h-section">
        <div className="h-wrap">
          <div className="h-section-header h-reveal">
            ...
            <h2 className="h-section-title">A summit shaped<br />by its delegates.</h2>
          </div>

          <div className="h-vision-layout">
            <div className="h-vision-body">
              <p className="h-p h-reveal">
                In many traditional MUNs, delegates often feel disconnected from the agendas being discussed. The Samaagm Summit is designed to change that.
              </p>
              <p className="h-p h-reveal">
                Before the conference, a series of <strong>interactive voting rounds on Instagram</strong> allows participants to choose between carefully curated agenda options. These votes help determine which topics and committees will ultimately be featured at the summit.
              </p>
              <blockquote className="h-quote h-reveal">
                <span className="h-quote-mark" aria-hidden>"</span>
                A conference that is not just <em>for</em> the delegates — but <em>shaped with them</em>.
            ...
            <div className="h-vision-steps">
              {[
                { n: "01", head: "Voted Agendas", body: "The community votes on Instagram to decide which topics get debated at each committee." },
                { n: "02", head: "Curated Experiences", body: "From music selection to event format — participants influence the environment they walk into." },
              ].map((s, i) => (
                <div key={s.n} className="h-vision-step h-reveal" style={{ transitionDelay: `${i * 0.1}s` }}>
      ...
      <section id="founders" className="h-section">
        <div className="h-wrap">
          <div className="h-section-header h-reveal">
            ...
            <h2 className="h-section-title">Founded by students,<br />built for students.</h2>
          </div>

          <div className="h-founders-editorial">
            {[
              { initials: "SK", name: "Somya Khandare", role: "CEO & Founder", age: "15" },
              { initials: "AB", name: "Arjav Badjatya", role: "CMO & Founder", age: "16" },
            ].map((f, i) => (
              <div key={f.name} className={`h-founder-row h-reveal${i === 0 ? " h-founder-row--first" : ""}`} style={{ transitionDelay: `${i * 0.12}s` }}>
                ...
                <div className="h-founder-details">
                  ...
                  <h3 className="h-founder-name-big"
                    onClick={f.name === "Arjav Badjatya" ? handleArjavClick : undefined}
                    style={f.name === "Arjav Badjatya" ? { cursor: "default", userSelect: "none" } : undefined}>
                    {f.name}
                  </h3>
                </div>
      ...
      <footer className="h-footer">
        ...
        <div className="h-wrap h-footer-inner">
          ...
          <div className="h-footer-links">
            <a href="https://www.instagram.com/thesamaagmsummit.tss" target="_blank" rel="noopener noreferrer" className="h-footer-link">
